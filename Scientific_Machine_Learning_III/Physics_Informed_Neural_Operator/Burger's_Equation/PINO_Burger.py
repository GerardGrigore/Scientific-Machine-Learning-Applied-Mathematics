...
# This algorithm gathers the functions that will be used to design a PINO to solve
...
# to train our supervised/semi-supervised PINO learning problem afterward.
...
def Burgers_Solver_One_Dimension(Initial_Function_Solution,
    ...
    # Number of step time:
    Number_Step_Times = int(np.round(Time_Final/Delta_Time_Tau))
        
    # Temporary half-delta-time variable declaration:
    Delta_Time_Tau_Half = 0.5*Delta_Time_Tau
    
    # To save the temporal trajectory too:
    Solution_Trajectory = []
    Solution_Trajectory.append(Function_Solution_Current.copy())
    Time_Trajectory = []
    Time_Trajectory.append(0.0)
    
    # Successions of diffusion and convection steps:
    for index_Time_Step in range(1,Number_Step_Times + 1):
        ...
        # Save whenever specified:
        if index_Time_Step % Save_Every == 0:
            Solution_Trajectory.append(Function_Solution_Current.copy())
            Time_Trajectory.append(index_Time_Step*Delta_Time_Tau)
        
    # Then output of the function:
    return np.array(Time_Trajectory),np.array(Solution_Trajectory)
...
def Generate_Burgers_PDE_Dataset(Number_Samples,Number_Points=256,Viscous_Coefficient=0.1,
                                 Delta_Time_Tau=1e-4,Alpha_Coefficient=2.0,
    ...
    Number_Time_Points = int((int(np.round(Time_Final/Delta_Time_Tau)))/Save_Every) + 1 # Since in Burgers_Solver_One_Dimension, the output
    # arrays Time_Trajectory and Function_Current_Sample_Output_Solver_Solved are initialized with an element, resulting in Number_Time_Points + 1 total
    # element and not Number_Time_Points.

    # Inputs and Outputs elements storage initialization:
    Inputs_Function = np.zeros((Number_Samples,Number_Points),dtype=np.float64)
    Outputs_Function = np.zeros((Number_Samples,Number_Time_Points,Number_Points),dtype=np.float64)
    ...
    for index_Number_Samples in range(Number_Samples):
        ...
        # Burger's equation:
        Time_Trajectory,Function_Current_Sample_Output_Solver_Solved = Burgers_Solver_One_Dimension(Function_Initial_Condition_Current_Sample,
    ...
    return One_Dimension_Space_Grid_X,Inputs_Function,Outputs_Function,Time_Trajectory
...
class Projection_Operation(nn.Module):
    def __init__(self,Dimension_Width=64,Dimension_Expressiveness=128,Number_Time_Points=21):
        super().__init__()
        self.Dimension_Width = Dimension_Width
        ...
        self.Projection_Matrix = nn.Sequential(nn.Linear(Dimension_Width,Dimension_Expressiveness),
                                               nn.GELU(),
                                               nn.Linear(Dimension_Expressiveness,Number_Time_Points))
...
class Fourier_Neural_Operator_Overall(nn.Module):
    def __init__(self,Dimension_Width=64,Mode_Maximal=32,Dimension_Expressiveness=128):
        super().__init__()
        # Global FNO:
        self.FNO_Network = nn.Sequential(Lifting_Operation(Dimension_Width=Dimension_Width),
                                         Fourier_Layer(Dimension_Width=Dimension_Width,Mode_Maximal=Mode_Maximal),
                                         Fourier_Layer(Dimension_Width=Dimension_Width,Mode_Maximal=Mode_Maximal),
                                         Fourier_Layer(Dimension_Width=Dimension_Width,Mode_Maximal=Mode_Maximal),
                                         Fourier_Layer(Dimension_Width=Dimension_Width,Mode_Maximal=Mode_Maximal),
                                         Projection_Operation(Dimension_Width=Dimension_Width,Dimension_Expressiveness=128))
        
    def forward(self,Input_Point):
        Input_Point = Input_Point.unsqueeze(-1) # Of the shape: (Batch_Size,Number_Grid_Points,1)
        Output = self.FNO_Network(Input_Point)  # Of the shape: (Batch_Size,Number_Grid_Points,Number_Time_Points)
        Output = Output.permute(0,2,1)          # Of the shape: (Batch_Size,Number_Time_Points,Number_Grid_Points)
        return Output.squeeze(-1)
    
# ---------------------------------------------------------------------------------------------
# 2) Various function declaration and Losses computation:
# ---------------------------------------------------------------------------------------------
...
def Compute_Loss_PDE(Initial_Function,Output_Function_FNO,Delta_Time,Number_Grid_Points,Viscous_Coefficient):
    
    # Coefficient Laplacian-like for the PDE cost function:
    Beta_PDE_Coefficient = 1.0
    
    # Compute the partial time derivative of the output function:
    Time_Partial_Derivative_Output_Function = (Output_Function_FNO[:,1:,:] - Output_Function_FNO[:,:-1,:])/Delta_Time
    
    # Operations to compute the spatial derivatives:
    # Modes frequency:
    Frequency_Modes_Grid = torch.fft.fftfreq(Number_Grid_Points,d=1.0/Number_Grid_Points)
    Frequency_Modes_Grid = 2*torch.pi*Frequency_Modes_Grid
    Frequency_Modes_Grid = Frequency_Modes_Grid.to(Output_Function_FNO.device) # To have the modes variable on the same
    # device as the output function.
    Frequency_Modes_Grid = Frequency_Modes_Grid.view(1,1,Number_Grid_Points)
    
    # Output function in spectral domain:
    Output_Function_FFT = torch.fft.fft(Output_Function_FNO,dim=2)
    # Size compatibility:
    Output_Function_FFT = Output_Function_FFT[:,:-1,:]
    # First partial derivative in function of the spatial dimension:
    Space_First_Partial_Derivative_Output_Function = torch.fft.ifft(1j*Frequency_Modes_Grid*Output_Function_FFT,dim=2).real
    # Second partial derivative in function of the spatial dimension:
    Space_Second_Partial_Derivative_Output_Function = torch.fft.ifft(-(Frequency_Modes_Grid**2)*Output_Function_FFT,dim=2).real
    # Squared output function space first derivative:
    Output_Squared = Output_Function_FNO**2
    Output_Squared = Output_Squared[:,:-1,:]
    Output_Squared_FFT = torch.fft.fft(Output_Squared,dim=2)
    First_Derivative_Output_Squared_FFT = 1j*Frequency_Modes_Grid*Output_Squared_FFT
    First_Space_Derivative_Output_Time = torch.fft.ifft(First_Derivative_Output_Squared_FFT,dim=2).real
    
    # Computation of the Loss:
    Loss_PDE_Inner_Term = Viscous_Coefficient*Space_Second_Partial_Derivative_Output_Function - 0.5*First_Space_Derivative_Output_Time 
    Loss_PDE_Term_1 = Time_Partial_Derivative_Output_Function - Loss_PDE_Inner_Term
    Loss_PDE_Term_2 = Output_Function_FNO[:,0,:] - Initial_Function
    Loss_PDE = torch.mean(Loss_PDE_Term_1**2) + Beta_PDE_Coefficient * torch.mean(Loss_PDE_Term_2**2)
    
    return Loss_PDE

# Operator loss for fine-tuning:
def Loss_Operator_Fine_Tuning(Operator_Current_Epoch_Function_Of_Input_Function_Optimal,
                              Operator_Initial_Epoch_Function_Of_Input_Function_Optimal):
    # L² norm computation error:
    Operator_Error = torch.linalg.norm(Operator_Current_Epoch_Function_Of_Input_Function_Optimal - \
                                       Operator_Initial_Epoch_Function_Of_Input_Function_Optimal)
...
def Main_Function():
    ...
    Fine_Tuning_Loss_Storage = []
    Fine_Tuning_Loss_PDE_Storage = []
    ...
    Number_Modes_Maximal_Function_Burger = 64
    Is_Article_Method_Initial_Conditions = 1
    #Lamda_PDE_Coefficient = 0.001
    Lamda_PDE_Coefficient = 0.1
    Alpha_Fine_Tuning = 1.0
    Save_Every = 100
    Time_Final = 0.2
    Time_Step_Trajectory = Delta_Time_Tau*Save_Every
    Is_Fine_Tuning_Loop = 0
    Number_Epochs_Fine_Tuning = 50
    # Width_NO = 64
    Width_NO = 128
    ...
    Outputs_Function_Training,\
    Time_Trajectory_Training = Generate_Burgers_PDE_Dataset(Number_Samples=1000,
    ...
    Outputs_Function_Testing,\
    Time_Trajectory_Testing = Generate_Burgers_PDE_Dataset(Number_Samples=200,
                                                           Number_Points=Number_Points,
                                                           Viscous_Coefficient=Viscous_Coefficient,
                                                           Delta_Time_Tau=Delta_Time_Tau,
                                                           Alpha_Coefficient=2.5,
    ...
    # Model of FNO:
    Model = Fourier_Neural_Operator_Overall(Dimension_Width=Width_NO,
                                            Mode_Maximal=Number_Modes_Maximal_Function_Burger,
                                            Dimension_Expressiveness=128)

    # Loss function choice:
    Loss_Data_Function = nn.MSELoss()
    
    # Optimizer choice:
    Optimizer = torch.optim.Adam(Model.parameters(),
                                 lr=0.001) # Note that the learning rate might be settled to be halved every 100 epochs as in the article.

    # Scheduler for the PINO:
    ...
    for index_Epoch in range(1,Number_Epochs + 1):
        ...
        # Current Loss_Data_Function at the current epoch initial value:
        Epoch_Loss = 0.0
        Epoch_Loss_Data = 0.0
        Epoch_Loss_PDE = 0.0
        ...
        for Inputs_Function_Batch_Training,Outputs_Function_Batch_Training in Train_Loader:
            ...
            Output_PINO = Model(Inputs_Function_Batch_Training)
            
            # PDE Loss calculation:
            Loss_PDE = Compute_Loss_PDE(Inputs_Function_Batch_Training,
                                        Output_PINO,
                                        Time_Step_Trajectory,
                                        Number_Grid_Points,
                                        Viscous_Coefficient)
            
            # Loss function calculation on the overall batch:
            Loss_Data = Loss_Data_Function(Output_PINO,Outputs_Function_Batch_Training)
            
            # Warm-up without PDE constraint:
            if index_Epoch <= 50:
                Lambda_Current_PDE = 0.0
            else:
                Lambda_Current_PDE = Lamda_PDE_Coefficient 
            
            # Total loss update:
            Loss = Loss_Data + Lambda_Current_PDE*Loss_PDE
            ...
            # Calculation of the current Loss_Data_Function of the current epoch. Based on the fact that the Pytorch Loss_Data_Function calculation is:
            ...
            Epoch_Loss_Data += Loss_Data.item()*Inputs_Function_Batch_Training.shape[0]
            Epoch_Loss_PDE += Loss_PDE.item()*Inputs_Function_Batch_Training.shape[0]
        ...
        Epoch_Loss_PDE /= len(Train_Loader.dataset)
        Epoch_Loss_Data /= len(Train_Loader.dataset)
        
        # Storage of the loss calculations:
        Loss_Storage.append(Epoch_Loss)
        Loss_PDE_Storage.append(Epoch_Loss_PDE)
        Loss_Data_Storage.append(Epoch_Loss_Data)
        ...
        if index_Epoch % 2 == 0 or index_Epoch == 1:
            ...
            print(f"Epoch {index_Epoch:4d} | "
                  ...
                  f"Loss PDE: {Epoch_Loss_PDE:.4f} | "
                  f"Loss data: {Epoch_Loss_Data:.4f} | "
    ...
    torch.save(Model.state_dict(),"Burger_PINO_Trained_1000_Samples_200_Epochs_Lambda_PDE_01_64_128.pth")
    # Save of the training history:
    np.savez("Burger_PINO_Training_History.npz",
             Loss_Storage=Loss_Storage,
             Loss_PDE_Storage=Loss_PDE_Storage,
             Loss_Data_Storage=Loss_Data_Storage,
    ...
    # Fine-tuning loop if option activated:
    if Is_Fine_Tuning_Loop == 1:
        # Evaluation mode:
        Model.eval()
        for index_Testing_Inputs in range(1): # For a First test.
            # Take the current reference solution:
            Input_New_Current = Inputs_Function_Testing_Pytorch_X[index_Testing_Inputs:index_Testing_Inputs + 1] # To be compatible
            # With the shape/size of the Model.
            with torch.no_grad():
                # To be calculated without gradient operation becasue is the prediction reference of the already trained PINO:
                Reference_Solution_Current = Model(Input_New_Current)
            # Define the fine-tuned model:
            Fine_Tuned_Model = copy.deepcopy(Model) # So that Model stays intact.
            # Optimizer definition:
            Optimizer_Fine_Tuning = torch.optim.Adam(Fine_Tuned_Model.parameters(),lr=1e-4) # On the Fine tuned model.
            # MSE Loss defition:
            Loss_Fine_Tuning = nn.MSELoss()
            for index_Epoch_Fine_Tuning in range(1,Number_Epochs_Fine_Tuning  + 1):
                Output_PINO_Fine_Tuning = Fine_Tuned_Model(Input_New_Current)
                # PDE Loss calculation:
                Loss_PDE_Fine_Tuning = Compute_Loss_PDE(Input_New_Current,
                                                        Output_PINO_Fine_Tuning,
                                                        Time_Step_Trajectory,
                                                        Number_Grid_Points,
                                                        Viscous_Coefficient)
                # Operator loss calculation:
                Loss_Operator = Loss_Fine_Tuning(Output_PINO_Fine_Tuning,Reference_Solution_Current)
                # Total loss calculation:
                Loss_Total = Loss_PDE_Fine_Tuning + Alpha_Fine_Tuning*Loss_Operator
                # Zero setlling of the previous weights gradients:
                ...
                Epoch_Loss_Fine_Tuning = Loss_Total.item()
                # Storage of the loss:
                Fine_Tuning_Loss_Storage.append(Epoch_Loss_Fine_Tuning)
                Fine_Tuning_Loss_PDE_Storage.append(Loss_PDE_Fine_Tuning.item()) # .item() to store the float value and not the 
                # Pytorch tensor.
                
                if index_Epoch_Fine_Tuning % 2 == 0 or index_Epoch_Fine_Tuning == 1:
                    # Print of the current loss of the fine-tuning loop training:
                    print(f"Epoch Fine Tuning {index_Epoch_Fine_Tuning:4d} | "
                          f"Loss Fine Tuning: {Epoch_Loss_Fine_Tuning:.4f} | ")
                    
        # Save of the training parameters:
        torch.save(Fine_Tuned_Model.state_dict(),"Burger_PINO_Fine_Tuned.pth")
        # Save of the training history:
        np.savez("Burger_PINO_Fine_Tuning_History.npz",
                 Fine_Tuning_Loss_Storage=Fine_Tuning_Loss_Storage,
                 Fine_Tuning_Loss_PDE_Storage=Fine_Tuning_Loss_PDE_Storage)               
                    
        # Plot of the evolution of the Loss during training:
        Epochs_Fine_Tuning_Vector = np.linspace(1,Number_Epochs_Fine_Tuning,Number_Epochs_Fine_Tuning,dtype=np.int16)
        plt.figure()
        plt.plot(Epochs_Fine_Tuning_Vector,Fine_Tuning_Loss_PDE_Storage,'-r')
        plt.axis("equal")
        plt.xlabel('Epoch number fine tuning (-)')
        plt.ylabel('Total loss of the Epoch (-)')
        plt.title('Loss evolution during fine-tuning of the model')
        plt.show()
